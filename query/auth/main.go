...
func main() {
	...
	var (
		thriftAddr       = fs.String("thrift.addr", ":39020", "Address for Thrift server")
	...
	client := etcd.EtcdReigistryClient{
		etcd.EtcdRegistryConfig{
			ServiceName:  banerwaiglobal.ETCD_KEY_MICROS_QUERY_AUTH,
	...
	go func() {
		...
		errc <- thrift.NewTSimpleServer4(
			thriftauth.NewAuthServiceProcessor(thriftBinding{svc}),
			transport,
			transportFactory,
			protocolFactory,
		).Serve()
