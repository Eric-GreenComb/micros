...
// ResumeCollection Resume表的Collection对象
var ResumeCollection *mgo.Collection
...
func main() {
	...
	var (
		thriftAddr       = fs.String("thrift.addr", ":36070", "Address for Thrift server")
	...
	ResumeCollection = Session.DB(*mongodbDbname).C("resume") //数据库名称
	...
	client := etcd.ReigistryClient{
		etcd.RegistryConfig{
			ServiceName:  banerwaiglobal.EtcdKeyMicrosCommandResume,
	...
	go func() {
		...
		errc <- thrift.NewTSimpleServer4(
			thriftresume.NewResumeServiceProcessor(thriftBinding{svc}),
			transport,
			transportFactory,
			protocolFactory,
		).Serve()
