...
import (
	...
	"github.com/banerwai/micros/command/token/thrift/gen-go/token"
	"math"
	"net"
	"net/url"
	"os"
...
func Usage() {
	...
	fmt.Fprintln(os.Stderr, "  i64 VerifyToken(string token, i64 ttype)")
	fmt.Fprintln(os.Stderr)
	os.Exit(0)
}
...
func main() {
	...
	client := token.NewTokenServiceClientFactory(trans, protocolFactory)
	...
	case "VerifyToken":
		if flag.NArg()-1 != 2 {
			fmt.Fprintln(os.Stderr, "VerifyToken requires 2 args")
			flag.Usage()
		}
		argvalue0 := flag.Arg(1)
		value0 := argvalue0
		...
		value1 := argvalue1
		fmt.Print(client.VerifyToken(value0, value1))
		fmt.Print("\n")
		break
	case "":
		Usage()
