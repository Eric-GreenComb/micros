...
import (
	...
	thriftrender "github.com/banerwai/micros/command/render/thrift/gen-go/render"
)
...
func main() {
	...
	var (
		thriftAddr       = fs.String("thrift.addr", ":6030", "Address for Thrift server")
	...
	go func() {
		...
		errc <- thrift.NewTSimpleServer4(
			thriftrender.NewRenderServiceProcessor(thriftBinding{svc}),
			transport,
			transportFactory,
			protocolFactory,
		).Serve()
