...
func main() {
	...
	var (
		thriftAddr       = fs.String("thrift.addr", ":36100", "Address for Thrift server")
	...
	BillingCollection = Session.DB(*mongodbDbname).C("billing") //数据库名称
	...
	client := etcd.EtcdReigistryClient{
		etcd.EtcdRegistryConfig{
			ServiceName:  banerwaiglobal.ETCD_KEY_MICROS_COMMAND_ACCOUNT,
	...
	go func() {
		...
		errc <- thrift.NewTSimpleServer4(
			thriftaccount.NewAccountServiceProcessor(thriftBinding{svc}),
			transport,
			transportFactory,
			protocolFactory,
		).Serve()
