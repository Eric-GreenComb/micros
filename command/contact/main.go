...
// ContactCollection Contact表的Collection对象
var ContactCollection *mgo.Collection
...
func main() {
	...
	var (
		thriftAddr       = fs.String("thrift.addr", ":36090", "Address for Thrift server")
	...
	ContactCollection = Session.DB(*mongodbDbname).C("contact") //数据库名称
	...
	client := etcd.ReigistryClient{
		etcd.RegistryConfig{
			ServiceName:  banerwaiglobal.EtcdKeyMicrosCommandContact,
	...
	go func() {
		...
		errc <- thrift.NewTSimpleServer4(
			thriftcontact.NewContactServiceProcessor(thriftBinding{svc}),
			transport,
			transportFactory,
			protocolFactory,
		).Serve()
